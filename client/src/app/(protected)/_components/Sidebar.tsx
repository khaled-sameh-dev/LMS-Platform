...
// app/(protected)/_components/SimplifiedSidebar.tsx
"use client";
...
export default function SimplifiedSidebar() {
  ...
  const getBottomNavItems = () => {
    const common = [
      ...
      },
      {
        label: "Settings",
        icon: Settings,
        path: `/${userRole.toLowerCase()}/settings`,
      },
    ];
      ...
      return [
        {
          ...
          icon: CreditCard,
          path: "/instructor/payment-methods",
        },
        ...common,
      ];
    }
  ...
  const sidebarContent = (
    <div className="h-full flex flex-col bg-secondry-blue">
      {/* Header */}
      <div className="px-6 py-4 border-b border-white/10">
        <div className="flex items-center justify-between">
          {!isCollapsed && (
            <motion.div
              ...
              className="flex items-center gap-3"
            >
              <div className="w-10 h-10 bg-main-blue rounded-lg flex items-center justify-center ">
                <GraduationCap className="w-6 h-6 " />
              </div>
              <div>
                <h2 className="text-white font-bold text-lg">LearnHub</h2>
                <p className="text-dirty-grey text-xs capitalize">
      ...
      {!isCollapsed && (
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <div className="relative">
              <img
                src={user?.imageUrl || "https://i.pravatar.cc/150?img=33"}
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={() => setIsMobileOpen(false)}
            className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40  block lg:hidden"
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <motion.aside
            ...
            exit={{ x: "-100%" }}
            transition={{ type: "spring", damping: 25 }}
            className="fixed left-0 top-0 bottom-0 w-80 bg-secondry-blue z-50 hidden lg:block shadow-2xl"
          >
            {sidebarContent}
          </motion.aside>
        )}
